...
def admin_lense_add(request):
    if request.method == "POST":
        ...
        lense = Lense.objects.create(p_name=name, p_desc=desc, p_quantity=quantity, p_price=price, p_image=image, cat=cat, seller=seller)
        lense.save()
    ...
    return render(request, 'admin_lenseadd.html', {"cat": cat, "sell": seller})
...
def seller_register(request):
    if request.method == 'POST':
        ...
        if first_name and last_name and username and password and mobile_phone:
            ...
            from_email = 'jan.23pmc127@mariancollege.org'  # Change this to your email
            to_email = email
...
def low_quantity_products(request):
    ...
    low_quantity_count = low_quantity_lenses.count()  # Count the number of low quantity lenses
    return render(request, 'low_quantity_products.html', {'low_quantity_count': low_quantity_count, 'low_quantity_lenses': low_quantity_lenses})
    ...
    if 'user_id' in request.session:
        try:
            ...
            send_mail(
                ...
                ['jan.23pmc127@mariancollege.org'],
                fail_silently=False,
            )
